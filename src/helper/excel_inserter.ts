...
export function excelInserter({
    ...
    sheets.map(sheet => {
        ...
        sheet.headers.map((header, index) => {
            ws.cell(currentRow, index + 1).string(header.title).style(styleHeader);
            if (header.width) {
        ...
        sheet.data.map((it, idx) => {
            ...
            sheet.headers.map((header, index) => {
                ...
                if ((typeof cellContent === 'undefined' || cellContent?.length === 0) && typeof header.defaultValue !== 'undefined') {
